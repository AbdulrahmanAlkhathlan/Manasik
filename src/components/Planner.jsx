...
const Planner = () => {
  ...
  const allCountries = Country.getAllCountries()
    .filter(country => country.name !== "Israel")  // Remove Israel
    .map((country) => ({
  ...
  const cityOptions = cities.map((city) => {
    const cleanName = city.name.split(",")[0].trim(); // remove state after comma
  ...
  useEffect(() => {
    ...
    if (!validOptions.includes(transportation)) {
      setTransportation(validOptions[0]); // auto-set to first valid
    }
